package com.pawtuah.mlb.estrogen;

import com.mojang.logging.LogUtils;
import com.pawtuah.mlb.block.ModBlocks;
...
@Mod(EstrogenMod.MODID)
public class EstrogenMod
{
    // Define mod id in a common place for everything to reference
    public static final String MODID = "mlbestrogen";
    ...
    public static final RegistryObject<Item> ESTROGEN_ITEM = ITEMS.register("estrogen",
            () -> new ItemNameBlockItem(EstrogenMod.ESTROGEN_CROP.get(), new Item.Properties().food(new FoodProperties.Builder().effect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 600000, 2, false, false), 1f).effect(new MobEffectInstance(MobEffects.JUMP, 600000, 2, false, false), 1f).alwaysEat().fast().nutrition(5).saturationMod(2f).build())));

    // Creates a creative tab with the id "examplemod:example_tab" for the example item, that is placed after the combat tab
    public static final RegistryObject<CreativeModeTab> ESTROGEN_TAB = CREATIVE_MODE_TABS.register("mlbestrogen", () -> CreativeModeTab.builder()
            .withTabsBefore(CreativeModeTabs.COMBAT)
            .icon(() -> ESTROGEN_ITEM.get().getDefaultInstance())
            .title(Component.literal("Estrogen Mod"))
    ...
    public EstrogenMod(FMLJavaModLoadingContext context)
    {
        ...
        context.registerConfig(ModConfig.Type.COMMON, com.pawtuah.mlb.estrogen.Config.SPEC);
    }
    ...
    private void commonSetup(final FMLCommonSetupEvent event)
    {
        ...
        if (com.pawtuah.mlb.estrogen.Config.logDirtBlock)
            LOGGER.info("DIRT BLOCK >> {}", ForgeRegistries.BLOCKS.getKey(Blocks.DIRT));

        LOGGER.info(com.pawtuah.mlb.estrogen.Config.magicNumberIntroduction + com.pawtuah.mlb.estrogen.Config.magicNumber);

        com.pawtuah.mlb.estrogen.Config.items.forEach((item) -> LOGGER.info("ITEM >> {}", item.toString()));
    }
